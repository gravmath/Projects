...
void main(void)
{
  ...
  //Open output files
  rhs = fopen("rhs.txt","w");
  log   = fopen("run_rhs.log","w");

  //Get system date and time and output to the log file
  _dos_getdate(&sys_d);
  _dos_gettime(&sys_t);
  fprintf(log,"**********Subscibe Only Geodesics/RHS Runs**********\n\n");
  ...
  fprintf(rhs,"%1.16g %1.16g %1.16g %1.16g %1.16g %1.16g",
          dstate[0],dstate[1],dstate[2],dstate[3],dstate[4],dstate[5],dstate[6]);
...
//  fprintf(mojo,"m = %d, n = %d, o = %d, Num = %d\n",m,n,o,Num);
